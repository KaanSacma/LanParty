...
def isOver(pos1, pos2):
    if pos2[0] < pos1[0] < pos2[0] + 190:
        if pos2[1] < pos1[1] < pos2[1] + 49:
            return True
    return False
...
def game_over():
    ...
    display_draw = font.render("It's a Draw", True, (255, 255, 255))

    displayQuit = font.render("Quit", True, (255, 255, 255))
    button_background = pygame.image.load("./asset/blue_button.png")
    rect = button_background.get_rect()
    rect.left, rect.top = [(WIDTH / 2) - 95, 380]
    screen.blit(button_background, rect)
    screen.blit(displayQuit, [(WIDTH / 2) - 38, 385])
    ...
    if score_value > score2_value:
        ...
        screen.blit(display_draw, ((WIDTH / 2) - 80, 330))
    canPlay = False
...
while 1:
    ...
    for event in py.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if isOver(mousePos, [(WIDTH / 2) - 95, 380]) and canPlay == False:
                quitGame()
